...
int findDirection(int row_position, int col_position, SIZE size, int *(arr)[size.col], int pre_row_compare) {
    ...
    int search_map_RECUR(int** map, int m, int n) {
    if (map[m][n] == 0) return 0;
    if (m == 0 && n == 0) return 1;
    
    if (m > 0 && n > 0) return search_map_RECUR(map, m - 1, n) + search_map_RECUR(map, m, n - 1);
    else if (m > 0 && n <= 0) return search_map_RECUR(map, m - 1, n) + 0;
    else if (m <= 0 && n > 0) return search_map_RECUR(map, m, n - 1)
}
